...
**结果**：`src/renderer/styles.css` **4502 → 1497 行（−67%）**，0 个 `.vue` 带 `<style>` 变成 15 个组件各有
...
| 两处"两个页面共用一套"的      | 「共享的向导零件」323（向导 ↔ 环境自检详情层）、「环境自检（共享部分）」100（`.env-op*` ↔ 向导）、「终端类页面」118（`.term-host` 基础规则 ↔ 两路终端） | ~541 |
