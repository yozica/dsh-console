...
**换一个环境接手、或在新会话里继续**：先读 [`docs/handoff.md`](docs/handoff.md) —— 那一份是**当前进度快照**（最新发布、这条线上最近合了什么、说好了还没做的、在这里干活的最短路径）。它自带背景，不需要看到此前的对话。
...
src/
  ...
  renderer/             Vue 3 + Vite，产物 dist/renderer/
    ...
    state/              **跨页共享状态与相位机**：store（唯一的快照订阅）/ boot-lock / env-doctor /
                        env-wizard / env-layer / env-anchor / update-anchor / update-fake（开发态的
                        "假装更新相位"，见 7.17）/ restart-nav / restart-flow
