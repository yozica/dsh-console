...
src/
  ...
  renderer/             Vue 3 + Vite，产物 dist/renderer/
    ...
    panes/              八个页面组件
test/selftest.ts        128 项自检（`npm test`），不需要 Electron
...
| `npm test`                      | `tsx test/selftest.ts`（128 项，不需要 Electron、不启停任何进程）                       |
