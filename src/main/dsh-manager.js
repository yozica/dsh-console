 ...
 *  - 停止时先 Ctrl+C 优雅退出，超时再 taskkill /T /F 清进程树
 */
...
class DshManager extends EventEmitter {
  ...
  /**
   * 本应用启动的 dsh 的 PID。
   * 优先读 pty 对象上的实时值（ConPTY 就绪后才有值），读不到时用端口占用者兜底。
  ...
  async start({ allowAdopt = true } = {}) {
    ...
    try {
      ...
      // ConPTY 的 PID 要等就绪事件之后才有值，这里只报告"已拉起"，PID 随后按需读取
      const pidNow = Number(session.proc && session.proc.pid)
      this.log('info', pidNow > 0 ? `dsh 已拉起，PID ${pidNow}` : 'dsh 已拉起，等待 ConPTY 就绪后确认 PID…')
    } catch (error) {
  ...
  async stop(options = {}) {
    ...
    if (this.pty.has(this.sessionId)) {
      ...
      if (this.sessionAlive) {
        // 强杀前再解析一次 PID：ConPTY 刚就绪时 pty 上还没有值，端口占用者兜底
  ...
  /** 端口上监听者的 PID 就是本应用拉起的 dsh（ConPTY 未就绪时用它兜底） */
  ...
  async pollOnce() {
    ...
    try {
      ...
      if (wantsOwner) {
        ...
        // 自己的进程：PID 未知时（ConPTY 尚未就绪）用端口占用者补上，界面和强杀都要用
