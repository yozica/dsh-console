...
const entriesOf = (layer: PluginLayer): PluginEntry[] => entriesOfLayer(layer, treeLayers.value);
...
const visibleGroups = computed(() =>
  visibleGroupsOf(treeLayers.value, {
    layerFilter: layerFilter.value,
    onlyDisabled: onlyDisabled.value,
    onlyOverridden: onlyOverridden.value,
    query: query.value,
  }),
);

const shownEntries = computed(() => countEntries(visibleGroups.value));
  ...
  <div class="plugin">
    ...
    <template v-else>
      ...
      <PluginConfigView
        v-else
        :query="query"
        :only-overridden="onlyOverridden"
        ...
        :op-busy="opBusy"
        :live-by-id="liveById"
        :my-patch-path="myPatchPath"
        @update:query="query = $event"
        @toggle-overridden="onlyOverridden = !onlyOverridden"
        @toggle-disabled="onlyDisabled = !onlyDisabled"
        @clear-filters="clearFilters"
        @back-to-full="backToFull"
...
<style scoped>
/* 插件页（装配层）自己的样式（t48 样式分层）：原来在 styles.css 的「插件页（装配层）」
   一整节 728 行。留在全局表的是跨页面共用的零件（`.btn` / `.panel*` / `.banner` /
   `.spacer` / `.hint` / `.empty` / `.tag`…），这一页的 `.plugin*` / `.layer*` / `.op-*`
   等等都在这里。页面上没有 v-html，所以不需要 `:deep()`。 */

/* ============================================================ 插件页（装配层）
   这一页的版式跟归档页是一族的：上面一条 bar，下面两列（层栈 / 详情），
   第二个视图把两列换成一份"按层分组的配置清单"。
   视觉上只有两处是本页特有的：层栈的序号 + 选中竖线，以及"你的层"和
   "被覆盖 / 被禁用"这三种标签 —— 它们必须一眼分得开，否则用户会以为
   自己改的东西生效了（真实数据里 web-app 覆盖 base 的 25 条有 23 条是
   把下层关掉，不是改配置）。 */
...
/* 两个视图的切换：胶囊按钮，和本地 Shell 的标签同一形态 */
.plugin-views {
  display: inline-flex;
  gap: 2px;
  padding: 2px;
...
.plugin-view:hover:not(.active) {
  color: var(--ink-dim);
}

/* "没报错的错"：这是本页唯一用玫瑰色铺底的地方，因为它是唯一会静默
   吃掉用户改动的失败（patch 指向了不存在的条目 / 文件解析失败 / 装了却
   没形成层）。 */
.plugin-problems {
  flex: 0 0 auto;
  margin: 0 20px;
...
/* 两种"不形成层"（真普通依赖 / 被摘掉的 bundle）都在黄档：不会让 dsh 起不来，只是改动不生效。
   缺贡献（missing-layer）留在上面的红档 —— 那一种 dsh 启动时可能直接失败。 */
.plugin-problem-kind[data-kind='plain-dependency'],
.plugin-problem-kind[data-kind='suspended-bundle'] {
...
/* 本页改不了的那一层（机器级的 $DSH_HOME/cordis.patch.yml）：不给按钮，但把文件指出来 */
.plugin-problem-where {
  flex: 0 0 auto;
  color: var(--ink-faint);
  font-size: var(--t-xs);
  overflow-wrap: anywhere;
...
/* ---- 救援条：dsh 起不来 / 配置被改坏时，先把出路摆出来（不只是报错） ---- */
.plugin-rescue {
  display: flex;
...
/* 救援条里的备份清单：整行铺开（救援条本身是 flex 行） */
.plugin-rescue {
  flex-wrap: wrap;
}

.plugin-rescue-backups {
  flex: 1 1 100%;
  list-style: none;
  margin: 4px 0 0;
  padding: 8px 0 0;
