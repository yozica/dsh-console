 ...
 *
 *   node tools/changelog-extract.mjs v0.2.1      # v 前缀可有可无
 *
 * 找不到条目、或条目为空，一律以非零码退出 —— 发版时宁可直接失败，
 * 也不要发出一个没有说明的 Release（缘由见 CHANGELOG.md 顶部）。
 *
 * `extractChangelog` 是导出的纯函数，`test/selftest.js` 直接调它来测，
 ...
 * @param {string} version  形如 `0.2.1` 或 `v0.2.1`
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!version) {
    console.error('用法: node tools/changelog-extract.mjs <version>   例如 v0.2.1')
    process.exit(2)
  }
  ...
  try {
    ...
    console.error(`读不到 CHANGELOG.md：${error.message}`)
    process.exit(2)
  }
