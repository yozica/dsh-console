...
- 37 个条目 263 行搬走（`.dash*` / `.focus-*` / `.stats` / `.meta-*` / `.event-log*` / `.chart` / `.spark*` / `.command`，含两条媒体查询），全局表 3914 → 3649 行；留在全局表的是卡片零件（`.panel` / `.panel-head` / `.panel-block` / `.hint`）与 `.block-head` / `.block-note`（插件页也在用）。
